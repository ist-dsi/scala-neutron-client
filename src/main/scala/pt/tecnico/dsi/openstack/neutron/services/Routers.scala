...
import pt.tecnico.dsi.openstack.neutron.models.*
...
final class Routers[F[_] : Concurrent : Client](baseUri: Uri, session: Session)
  ...
  private def computeUpdatedExternalGatewayInfo(existing: Option[ExternalGatewayInfo], create: Option[ExternalGatewayInfo],
    ...
    (existing, create) match
      ...
      case (Some(_), None) =>
        ...
        if keepExistingElements then existing else
      ...
      case (Some(existingInfo), Some(createInfo)) =>
        ...
        for newExternalIp @ ExternalIp(subnetId, newIp) <- createInfo.externalFixedIps do
  ...
  override def defaultResolveConflict(existing: Router, create: Router.Create, keepExistingElements: Boolean, extraHeaders: Seq[Header.ToRaw]): F[Router] =
    val updated = Router.Update(
      ...
      distributed = if !create.distributed.contains(existing.distributed) then create.distributed else None,
      ha = if !create.ha.contains(existing.ha) then create.ha else None,
    )
  ...
  final class Operations(val id: String) { self =>
    ...
    private def routesOperation(routes: List[Route[IpAddress]], path: String): F[List[Route[IpAddress]]] =
      ...
      import pt.tecnico.dsi.openstack.neutron.models.given
      val encoder = Encoder[List[Route[IpAddress]]].mapJson(j => Json.obj("routes" -> j))
      val decoder = Decoder[List[Route[IpAddress]]].at("routes")
      put(wrappedAt = Some(name), routes, uri / id / path)(using encoder, decoder)
    ...
    def addInterface(subnet: Subnet[?]): F[Option[RouterInterface]] = addInterfaceBySubnet(subnet.id)
    def removeInterface(subnet: Subnet[?]): F[Unit] = removeInterfaceBySubnet(subnet.id)
  }
